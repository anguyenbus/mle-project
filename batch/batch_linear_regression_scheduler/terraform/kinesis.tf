resource "aws_kinesis_stream" "profile_update_input_from_batch_process" {
...
resource "aws_lambda_event_source_mapping" "example" {
  event_source_arn  = aws_kinesis_stream.profile_update_input_from_batch_process.arn
...
resource "aws_ssm_parameter" "profile_update_input_from_batch_process" {
  name        = "/anguyenbus/${var.id}/batch-linear-regression/d61/kinesis/profile-update"
  description = "Profile updates"
  type        = "String"
  value       = aws_kinesis_stream.profile_update_input_from_batch_process.name
