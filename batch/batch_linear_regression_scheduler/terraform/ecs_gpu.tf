...
resource "aws_ecs_cluster" "ecs_cluster_gpu" {
  ...
  tags = merge(local.tags, { Name = "${var.name}-batch-linear-regression" })
}
...
module "ecs_instance_profile_role_gpu" {
  ...
  role_name               = "${var.name}-batch-linear-regression-ecs-instance-role"
  create_role             = true
  ...
  tags = merge(local.tags, { Name = "${var.name}-batch-linear-regression-ecs-instance-role" })
}
...
module "autoscaling_ec2_instances_gpu" {
  ...
  name = "${var.name}-batch-linear-regression-ecs-instance"

  # Launch configuration
  lc_name = "${var.name}-batch-linear-regression-ecs-instance"
  ...
  # Auto scaling group
  create_asg                = true
  asg_name                  = "${var.name}-batch-linear-regression-ecs-instance"
  ...
  tags = concat(
    [
      {
        key                 = "Cluster"
        value               = "${var.name}-batch-linear-regression"
...
module "ecs_instance_sg_gpu" {
  ...
  name            = "${var.name}-batch-linear-regression-ecs-instances-sg"
  description     = "Security group for ${var.name} batch-linear-regression ecs instances."
  ...
  tags = merge(local.tags, { Name = "${var.name}-batch-linear-regression-ecs-instances-sg" })
}

resource "aws_security_group_rule" "rds_batch_linear_regression_sg" {
...
resource "aws_ssm_parameter" "ecs_cluster_name_gpu" {
  name        = "/anguyenbus/${var.id}/batch-linear-regression/processing-ecs-cluster/name"
...
resource "aws_ssm_parameter" "asg_gpu" {
  ...
  name        = "/anguyenbus/${var.id}/batch-linear-regression/processing-ecs-cluster/asg"
